...
def labeled_matrix_to_lists(doc_word, labels):
    """Convert a (sparse) matrix of counts into arrays of word and doc indices
    ...
    labels : array (D)
        array of labels for each document in doc_word

    Returns
    -------
    (WS, DS, YS) : tuple of two arrays
        WS[k] contains the kth word in the corpus
        DS[k] contains the document index for the kth word
        YS[k] contains the label of the document at DS[k]

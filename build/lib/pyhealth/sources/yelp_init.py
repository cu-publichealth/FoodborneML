...
def updateDBFromFeed(filename, geocode=True):
    ...
    with open(filename, 'rb') as infile: # for unzipped files
        ...
        for line in infile:
            ...
            street_address = street_address.lower() # for consistency
            ...
            for rev in biz['reviews']:
                ...
                # else:
                #     stmt = documents.insert().values(type=yelp_reviews.name)
                #     db.execute(stmt)
                #     docid = db.query(Document).order_by(Document.doc_id.desc()).first().doc_id
                #     stmt = yelp_reviews.insert().values(
                #                         rating=rev['rating'],
                #                         doc_id=docid,

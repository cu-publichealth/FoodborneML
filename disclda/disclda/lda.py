...
class DiscLDA:
    ...
    def __init__(self, n_topics, n_per=1, n_iter=2000, alpha=0.1, eta=0.01, random_state=None,
                 refresh=10):
        ...
        rng = disclda.utils.check_random_state(random_state)
    ...
    def fit(self, X, y):
        ...
        y : array, shape (n_samples)
            Labels for each sample.  Labels must be integers starting from 0
    ...
    def transform(self, X, max_iter=20, tol=1e-16):
        ...
        WS, DS = disclda.utils.matrix_to_lists(X)
    ...
    def _fit(self, X, y):
        ...
        y: array, shape(n_samples)
            array of labels for each data sample
        """
        random_state = disclda.utils.check_random_state(self.random_state)
    ...
    def _initialize(self, X, y):
        ...
        self.WS, self.DS, self.YS = WS, DS, YS = disclda.utils.labeled_matrix_to_lists(X, y)

        # create class, topics matrix
        C = np.unique(y)
        L = len(C)
        Ts = []
        for i in range(L):
            T = np.hstack((np.zeros(L*self.n_per), np.ones(self.n_topics - L*self.n_per)))
            T[i*self.n_per: (i+1)*(self.n_per)] = 1
            Ts.append(T)
        self.Ts = np.array(Ts).astype(np.intc)
        # Ts = {}
        # for i, c in enumerate(C):
        #     T = np.zeros((self.n_topics, self.n_topics))
        #     for j in range(self.n_per):
        #         T[i*self.n_per+j, i*self.n_per+j] = 1
        #     for j in range(1, self.n_topics-L*self.n_per+1):
        #         T[-j,-j] = 1
        #     print c
        #     Ts[c] = T
        # self.Ts = Ts.astype(np.intc)

        self.ZS = ZS = np.empty_like(self.WS, dtype=np.intc)
        np.testing.assert_equal(N, len(WS))
        for i in range(N):
            w, d, c = WS[i], DS[i], YS[i]
            z_new = i % n_topics
            # make sure it's not a topic it can't be
            T = Ts[c]
            if T[z_new] == 0:
                for z in range(z_new, self.n_topics):
                    if T[z] != 0:
                        z_new = z
                        break
    ...
    def loglikelihood(self):
        ...
        return disclda._lda._loglikelihood(nzw, ndz, nz, nd, alpha, eta)
    ...
    def _sample_topics(self, rands):
        ...
        #print np.dtype(self.Ts[0,0])
        #print np.dtype(self.YS[0])
        disclda._lda._sample_topics(self.WS, self.DS, self.ZS, self.YS, self.Ts, 
                                    self.nzw_, self.ndz_, self.nz_, alpha, eta, rands)

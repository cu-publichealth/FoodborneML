...
def updateDBFromFeed(filename, geocode=True):
    ...
    db = getDBSession(echo=True, autoflush=False)
    ...
    with open(filename, 'rb') as infile: # for unzipped files
        ...
        for line in infile:
            ...
            elif street_address in unloaded_locations.keys():
                location = unloaded_locations[street_address]
            else:
                location = None
            ...
            # didn't find location in db so make one
                ...
                unloaded_locations[street_address] = location # add to running list of locations
            db.add(location)
            #print street_address
            #if street_address in db_locations: print "Address added!"
            #else: print "Address not added!"
            ...
            for category in biz['categories']:
                ...
                elif category['alias'] in unloaded_categories.keys():
                    cat = unloaded_categories[category['alias']]
                else:
            ...
            if biz_num % upload_mod == 0:
                logger.info("Uploading Batch to DB....")
                db.commit()
